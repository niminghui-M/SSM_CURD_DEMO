...
<head>
    ...
    <!-- 不以/开始的相对路径，找资源时，会以当前资源路径为基准，经常容易出问题
         以/开始的相对路径，找资源时，会以服务器的路径为标准(http://localhost:3306)：需要加上项目名
...
<body>
    ...
    <script type="text/javascript">
        ...
        function validate_add_form() {
            //1、拿到要校验的数据，使用JQuery中的正则表达式
            var empName = $("#empName_add_input").val();
            var regName = /(^[a-zA-Z0-9_-]{6,16}$)|(^[\u2E80-\u9FFF]{2,5})/;
            if(!regName.test(empName)) {
                //alert("用户名必须是2-5位中文，或者6-16位英文和数组的组合");
                show_validate_msg("#empName_add_input","error","用户名必须是2-5位中文，或者6-16位英文和数组的组合");
                return false;
            } else {
            ...
            $.ajax({
                ...
                success:function (reslut) {
                    if(reslut.code == 100) {
                        ...
                        show_validate_msg("#empName_add_input","error",reslut.extend.va_msg);
        ...
        $("#check_all").click(function () {
            ...
            //我们这些dom原生的属性-prop    attr获取自定义属性的值
            //prop修改和读取dom原生属性的值
